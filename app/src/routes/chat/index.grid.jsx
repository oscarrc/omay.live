...
const Chat = () => { 
    ...
    return (
        <section className="grid grid-rows-2 md:grid-chat grid-flow-row md:grid-flow-col gap-4 w-full flex-1 max-h-display"> 
            {
                !isTextOnly &&
                <>
                    <div className="flex flex-col gap-4 row-span-2 relative">
                        <VideoBox source={remoteStream} className="aspect-4/3 max-h-full" />
                        <VideoBox source={localStream} className="w-1/4 md:w-auto absolute md:relative bottom-2 md:bottom-auto right-2 md:right-auto md:aspect-4/3 max-h-full" />
                    </div>
            ...
            <ChatBox 
                    className="scroll row-span-2 flex flex-col flex-1 gap-4 max-h-content overflow-y-auto md:min-w-3/4" 
                    messages={messages} 
                    status={status} 
