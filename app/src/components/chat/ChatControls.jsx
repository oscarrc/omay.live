...
const ChatControls = ({ onClick, onChange, onSubmit, state, disabled }) => {
    ...
    return (
        <div className="join w-full h-16">                   
            <button disabled={disabled} onClick={onClick} className={`join-item btn ${ state === 0 ? "btn-primary" : "bg-base-100"} h-full flex flex-col gap-1 min-w-24 rounded-none sm:rounded-lg`}>
            ...
            <button onClick={send} className="join-item btn h-full flex flex-col gap-1 bg-base-100 min-w-24 rounded-none sm:rounded-lg">{t("chat.send")} <span className="text-primary text-xs">Enter</span></button>
        </div>
    )
}
